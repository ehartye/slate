...
- `lib/components/Editor.svelte` — CodeMirror 6. Markdown loads synchronously (a static import, always bundled); every other recognized language (Python, JS, Rust, ...) loads asynchronously and lazily via `@codemirror/language-data`'s `LanguageDescription.matchFilename` + `.load()` — each language is its own dynamically-imported chunk (same lazy-loading approach as pdf.js), swapped into a shared `Compartment` once loaded so a non-markdown tab isn't stuck with no syntax highlighting (or, worse, markdown's own highlighting) for its actual language. Source-syntax highlighting is defined via two `HighlightStyle`s whose colors are `var(--…)`: one markdown-specific, one general-purpose (keywords/strings/comments/etc., reusing the exact same `--code-*` variables the Preview pane's highlight.js output uses — see `prose.css` — so a construct like a string or comment gets the same color in both places) so the editor follows the active theme either way.
...
- `lib/markdown.ts` — the single `markdown-it` instance with all plugins (task lists, footnotes, anchors+TOC, mark/sup/sub/kbd, GitHub alerts, KaTeX) plus highlight.js fence highlighting and front-matter → metadata-card rendering. Mermaid fences are emitted as `<pre class="mermaid">` for Preview to render. `html: true` — raw HTML embedded in a document is rendered rather than escaped (GitHub-style `<details>`, `<div align>`, inline `<span style>`), so **every render is piped through DOMPurify** before it reaches the DOM. A markdown file is untrusted input, and the "Open in browser" export (`export.ts`) writes a standalone file with no CSP at all, so the sanitizer — not the app's CSP — is the actual boundary here. Two non-obvious sanitizer settings: `ADD_ATTR: ['target', 'align']` (not in DOMPurify's default allowlist, but ubiquitous in hand-written markdown HTML), and `SANITIZE_DOM: false` — DOMPurify's DOM-clobbering guard otherwise strips any `id` whose value collides with a `document` property, which silently breaks heading anchors and TOC links for headings as ordinary as "Title", "Location", or "Images"; the guard only protects code doing named `document.<x>` lookups, which this app never does. Iframes/objects/embeds are force-forbidden regardless of profile.
...
- `tauri.conf.json`'s `security.csp` is enforced only in production (`tauri build`); it's not exercised by `tauri dev`, which serves the raw Vite dev response instead of the compile-time-hashed bundle. Test CSP changes against a real build. `style-src` needs `'unsafe-inline'` — the theme system injects an active theme's CSS via `style.textContent` at runtime (`theme.ts`), which compile-time hashing can't cover, and mermaid/KaTeX do the same for their own dynamic styles. `connect-src` needs `'self'` (in addition to the `ipc:`/`http://ipc.localhost` Tauri itself requires) — `export.ts`'s `collectThemeFontCss`/`collectMermaidScript` `fetch()` their own same-origin bundled assets (a theme's font files, mermaid's UMD bundle) to embed them into the "Open in browser" standalone export; without `'self'` these fetches are silently blocked by CSP (caught and swallowed, so the failure mode is just missing fonts/unrendered mermaid diagrams in the exported page, not a visible error) — confirmed directly by replaying this exact CSP string through a headless browser engine.
